...
let init_needed () : (Kdo.Concrete.I32.t, _) Result.t =
  if (!path_to_file = "") 
    then Ok (Kdo.Concrete.I32.of_int 0) (* 0 si il n'y a rien a faire *)
    else Ok (Kdo.Concrete.I32.of_int 1) (* 1 si on doit initialiser *)

let init () : (unit, _) Result.t = 
  if (!path_to_file = "")
    then raise (InternalError "impossible d'initialiser, aucun chemin n'est précisé")
        ...
        try (
          ...
          for _x=0 to max_x-1 do
            ...
            for y=0 to (String.length line)-1 do
              data_from_file := !data_from_file@[(int_of_char line.[y]) - 48] (* on retire -48 à la valeur pour avoir le vrai numéro, pas son code ascii *)
            done
          done ;
...
let load_next_point () : (Kdo.Concrete.I32.t * Kdo.Concrete.I32.t, _) Result.t =
  ...
  if (!index_of_data >= List.length !data_from_file) 
    then
